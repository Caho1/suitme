...
class OutfitTaskRepository:
    ...
    async def get_by_id(self, id: int) -> OutfitTask | None:
        """
        根据内部数据库 ID 获取任务
        ...
        """
        result = await self.session.execute(
            select(OutfitTask).where(OutfitTask.id == id)
        )
        return result.scalar_one_or_none()

    async def get_by_task_id(self, task_id: str) -> OutfitTask | None:
        """
        ...
        """
        result = await self.session.execute(
            select(OutfitTask).where(OutfitTask.task_id == task_id)
        )
        return result.scalar_one_or_none()
    ...
    async def update_status(
        ...
        await self.session.execute(
            update(OutfitTask)
            .where(OutfitTask.task_id == task_id)
            .values(**update_data)
        )
        await self.session.flush()
    ...
    async def get_pending_tasks(self) -> list[OutfitTask]:
        """
        ...
        """
        result = await self.session.execute(
            select(OutfitTask).where(
                OutfitTask.status.in_([
